...
TEST(Basic, Smoke)
{
    ...
    EXPECT_EQ(points.size(), point_set.size());
    for (const auto &point : points)
        EXPECT_EQ(point_set.count(point), static_cast<size_t>(1));
}
...
TEST(Basic, X)
{
    ...
    EXPECT_EQ(component_matters, true);
}
...
TEST(Basic, Distribution)
{
    ...
    // EXPECT_EQ ниже отработает успешно,
    //
    ...
    EXPECT_LE(pearson_stat, critical_value);
}