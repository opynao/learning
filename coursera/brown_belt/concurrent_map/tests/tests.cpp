...
TEST(Basic, ConstAccess)
{
    ...
    EXPECT_EQ(cm.BuildOrdinaryMap(), expected);
}

TEST(Basic, StringKeys)
{
...
TEST(Basic, UserType)
{
    ...
    for (int i = 0; i < 1000; ++i)
    {
        EXPECT_EQ(point_weight.At(Point{i, i}).ref_to_value, i);
    }

    const auto weights = point_weight.BuildOrdinaryMap();
    for (int i = 0; i < 1000; ++i)
    {
        EXPECT_EQ(weights.at(Point{i, i}), i);
    }
}
...
TEST(Basic, Has)
{
    ...
    EXPECT_EQ(const_map.Has(1), true);
    EXPECT_EQ(const_map.Has(2), true);
    EXPECT_EQ(!const_map.Has(3), true);
}