...
TEST(Basic, TestConcurrentUpdate)
{
    ...
    EXPECT_EQ(common_string.GetAccess().ref_to_value.size(), 2 * add_count);
}
...
TEST(Basic, TestProducerConsumer)
{
    ...
    EXPECT_EQ(consumer.get(), expected);
